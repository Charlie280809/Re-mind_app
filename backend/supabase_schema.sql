...
-- 	constraint favorite_pauses_user_id_pause_type_key unique (user_id, pause_type),
...
-- grant select, insert, delete on public.favorite_pauses to authenticated;

-- drop policy if exists select_own_favorites on public.favorite_pauses;
-- create policy select_own_favorites
-- 	on public.favorite_pauses
-- 	for select
-- 	using (user_id = auth.uid());

-- drop policy if exists insert_own_favorites on public.favorite_pauses;
-- create policy insert_own_favorites
-- 	on public.favorite_pauses
-- 	for insert
-- 	with check (user_id = auth.uid());

-- drop policy if exists delete_own_favorites on public.favorite_pauses;
